...
app.post('/api/bookings/guest', async (req, res) => {
  const { guestName, guestPhone, guestEmail, serviceId, stylistId, startTime } =
    req.body;
  try {
    const { data: guest } = await supabase
      .from('guests')
      .insert([
        { full_name: guestName, phone_number: guestPhone, email: guestEmail },
      ])
      .select()
      .single();
    const { data: service } = await supabase
    ...
    res
      .status(201)
      .json({
        message: 'Shadow Booking Created',
        guest,
        booking,
        price: finalPrice,
      });
...
app.put('/api/bookings/:id/complete', async (req, res) => {
  try {
    const { data: booking } = await supabase
      .from('bookings')
      .select('customer_id, services(base_price)')
      .eq('id', req.params.id)
      .single();
...
app.get('/api/analytics', async (req, res) => {
  try {
    ...
    const topStylist = Object.entries(stylistCounts).sort(
      (a, b) => b[1] - a[1]
    )[0] || ['None', 0];
