...
export default function StylistSchedule({ onBack }) {
  ...
  useEffect(() => {
    async function fetchMySchedule() {
      ...
      try {
        // 1. Fetch all stylists to find "Me"
        const stylistRes = await fetch('http://localhost:5000/api/stylists');
        const stylists = await stylistRes.json();
        
        // Match logged-in email to the stylist's email column
        // e.g. matching "senior@salon.com" to the Senior Stylist record
        ...
        // 2. Fetch ALL bookings and filter (Prototype approach)
        // In production, you'd have a specific /api/bookings/my endpoint
        const bookingRes = await fetch('http://localhost:5000/api/bookings');
        const allBookings = await bookingRes.json();

        // 3. Filter for MY ID only
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded shadow text-center max-w-md border-l-4 border-red-500">
          ...
          <p className="text-gray-600 mb-6">
            Your email ({user?.email}) is not linked to an active Stylist Profile.
          </p>
          <button 
            onClick={onBack} 
            className="bg-gray-200 px-4 py-2 rounded font-bold text-sm hover:bg-gray-300 transition"
          >
            Go Back
          </button>
        </div>
        ...
        <div className="flex justify-between items-end mb-8 border-b pb-4">
          <div>
            ...
            <p className="text-gray-500 mt-2">
              Logged in as: <span className="font-bold text-black">{stylistProfile.name}</span>
            </p>
          </div>
          <button 
            onClick={onBack} 
            className="text-xs font-bold uppercase underline hover:text-red-600 text-gray-400 transition"
          >
            Exit Portal
          </button>
        </div>
          ...
          {myBookings.length === 0 ? (
            ...
            <div>
              {myBookings.map((booking) => {
                ...
                // Helper to get client name (Registered User or Guest)
                  ...
                  <div key={booking.id} className="border-b last:border-0 border-gray-100 p-6 flex items-center hover:bg-gray-50 transition group">
                    ...
                    <div className="flex-grow pl-6">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="bg-black text-white text-[10px] px-2 py-0.5 rounded uppercase font-bold tracking-wider">
                          {serviceName}
                        </span>
                    ...
                    <div className="ml-4 opacity-0 group-hover:opacity-100 transition">
                      <span className="text-green-600 text-xs font-bold uppercase tracking-widest border border-green-200 px-3 py-1 rounded-full bg-green-50">
                        Active
                      </span>
                    </div>
                  </div>
